...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      // await !user.matchPasswords(password, user.password))
      return res.status(404).json({
        status: 400,
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    //3. Assign token
    const token = jwt.sign({ mail }, process.env.RESET_SECRET_STR, {
...
exports.ConnectionRequest = async (req, res) => {
  ...
  try {
    ...
    notification = await notification.populate("receiver", "name profilePic");
    //notification with populated data
    const { io } = require("../app"); 
    io.to(targetUserId).emit("recieveNotification", notification);
    ...
    // Populate sender,receiver before emitting
    notification = await notification.populate("sender", "name profilePic");
    notification = await notification.populate("receiver", "name profilePic");
...
exports.searchUsers = async (req, res) => {
  try {
    ...
    if (!nameQuery) {
      return res
        .status(400)
        .json({ status: "fail", message: "Missing search query" });
    }
